...
import { Job } from "@/src/common/interfaces/job.interface";
import { Question } from "@/src/common/interfaces/question.interface";
import { createJob, createQuestion, getJobs, getQuestions, removeQuestion, updateQuestion } from "../actions/interviesAction";
  ...
  extraReducers(builder) {
    builder
      ...
      .addCase(updateQuestion.fulfilled, (state, { payload }) => {
        const index = state.questions.findIndex((que) => que._id === payload.data._id);
        if (index !== -1) {
